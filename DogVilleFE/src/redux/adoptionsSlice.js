...
export const deleteAdoption = createAsyncThunk(
    ...
    async (adozioneId, { rejectWithValue }) => {
        try {
            const response = await fetch(`http://localhost:3001/adozioni/${adozioneId}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("Access Token")}`,
            ...
            if (!response.ok) {
                const errorData = await response.json();
                return rejectWithValue(errorData.message || "Errore durante l'eliminazione dell'adozione");
            }

            return adozioneId;
        } catch (error) {
            return rejectWithValue("Errore durante l'eliminazione dell'adozione: " + error.message);
        }
    }
);
...
export const addAdoptionDocument = createAsyncThunk(
    ...
    async ({ adoptionId, file }, { rejectWithValue }) => {
        try {
            ...
            const response = await fetch(`http://localhost:3001/adozioni/${adoptionId}/document`, {
                method: "PATCH",
                headers: {
                    "Authorization": `Bearer ${localStorage.getItem("Access Token")}`,
                },
            ...
            if (!response.ok) {
                const errorData = await response.json();
                return rejectWithValue(errorData.message || "Errore durante il caricamento del documento");
            }

            const documentUrl = await response.text();
            return { adoptionId, documentUrl };
        } catch (error) {
            return rejectWithValue("Errore durante il caricamento del documento: " + error.message);
        }
    }
);
...
export const fetchAdoptionState = createAsyncThunk(
    ...
    async (adoptionId, { rejectWithValue }) => {
        try {
            const response = await fetch(`http://localhost:3001/adozioni/${adoptionId}/state`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${localStorage.getItem("Access Token")}`,
            ...
            if (!response.ok) {
                const errorData = await response.json();
                return rejectWithValue(errorData.message || "Errore durante il recupero dello stato dell'adozione");
            }

            const data = await response.json();
            return data;
        } catch (error) {
            return rejectWithValue("Errore durante la fetch dello stato dell'adozione: " + error.message);
        }
    }
);
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(deleteAdoption.fulfilled, (state, action) => {
                state.status = 'succeeded';
                state.data = state.data.filter(adoption => adoption.id !== action.payload);
                state.totalElements -= 1;
            })
            ...
            .addCase(addAdoptionDocument.fulfilled, (state, action) => {
                state.status = 'succeeded';
                const adoptionIndex = state.data.findIndex(adoption => adoption.id === action.payload.adoptionId);
                if (adoptionIndex !== -1) {
