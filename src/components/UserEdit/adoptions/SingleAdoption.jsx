...
function SingleAdoption({ adoption, toggleNavbarVisibility }) {
  ...
  const handleSignature = (svgBlob) => {
    const formData = new FormData();
    formData.append("sign", svgBlob, "signature.svg");
    ...
    dispatch(
      ...
      .catch((err) => {
        console.error("Errore durante il salvataggio della firma:", err);
      });
  };
  ...
  const handleUploadDocument = async (event) => {
    ...
    dispatch(
      addAdoptionDocument({ adoptionId: adoption.id, file: selectedFile })
    );
  };
  ...
  const handleDownloadCertificate = useCallback(async () => {
    ...
    doc.text(
      `Data adozione: ${new Date(adoption.creationDate).toLocaleDateString()}`,
      10,
      60
    );
    doc.text(
      "Grazie per aver dato una casa amorevole al tuo nuovo amico!",
      10,
      80
    );
    doc.setFontSize(16);
    ...
    console.log(adoption.sign);

    if (adoption.sign) {
      try {
        const base64Image = await convertSvgToBase64(adoption.sign);
        doc.addImage(base64Image, "PNG", 10, 260, 50, 30);
      } catch (error) {
        console.error("Errore nella conversione della firma:", error);
      }
    }
  ...
  return (
    <div
      key={refreshKey}
      className="card lg:card-side bg-reddino shadow-xl overflow-hidden rounded-2xl p-5 relative"
    >
      {/* Bottone per eliminare l'adozione */}
      <button
        onClick={handleDeleteAdoption}
        className={`absolute top-2 right-2 z-10 bg-red-800 text-white rounded-full w-8 h-8 flex items-center justify-center hover:bg-red-700 transition-colors ${
          adoption.state === "ADOZIONE_COMPLETATA"
      ...
      <div className="card-body text-primary-color">
        ...
        <div className="flex justify-center gap-5 my-3 flex-wrap">
          <p className="text-center">
            ...
            {adoption.dog.age} anni
          </p>
          <p className="text-center">
            <span className="font-semibold">Taglia: </span>
            {adoption.dog.dogSize}
          </p>
          <p className="text-center">
          ...
          <p className="text-center">
            ...
            {adoption.dog.gender == "M" ? "Maschio" : "Femmina"}
          </p>
          <p className="text-center">
            <span className="font-semibold">Stato di salute: </span>
            {adoption.dog.healthState.replaceAll("_", " ")}
          </p>
        </div>
        ...
        <div className="flex justify-between flex-col lg:flex-row mt-3">
          ...
          <div className="flex flex-col gap-3">
            ...
            {adoption.visitDate && (
              <p className="">
                <span className="font-semibold">Data visita programmata: </span>
                {adoption.visitDate}
              </p>
            )}
            <p>
        ...
        {/* Pulsanti dinamici */}
        <div className="justify-end w-full flex mt-10">
          {renderButtonContent()}
        </div>
      </div>
      ...
      {showSignatureModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-lg p-6 shadow-lg w-11/12 sm:w-1/2">
            ...
            <DigitalSign
              name={meData.name + " " + meData.surname}
              onSignGenerated={handleSignature}
            />
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-lg p-6 shadow-lg w-11/12 sm:w-1/2">
            ...
            <p className="text-gray-600 mb-6">
              Sei sicuro di voler eliminare questa adozione per{" "}
              {adoption.dog.name}?
            </p>
            <div className="flex justify-end space-x-4">
              <button
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 !z-50">
          <div className="bg-white rounded-lg p-6 shadow-lg w-11/12 sm:w-1/2">
            ...
            <div className="flex gap-2 flex-col lg:flex-row items-center lg:items-start">
              ...
              />
              <div>
                <p className="text-primary-color mb-4 text-2xl font-bold ">
                  {adoption.dog.name.toUpperCase()}
                </p>
